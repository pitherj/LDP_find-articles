...
- **Last updated**: 2026-04-13 (see Addendum for post-hoc correction files)
...
## Addendum — Post-hoc Correction Files (2026-04-13)

Scripts 05–08 were added after the initial rater files were produced to correct five ineligible publications identified during rater review and one inaccessible article. The files below were created by those scripts. The original files (`rater_publications.csv`, `private/rater_key.csv`, and all script 03 outputs) are unchanged. See the README Addendum for full context.

The **active rater files** after all corrections are `rater_publications_final_v2.csv` and `private/rater_key_final_v2.csv`.

---

### `processed_data/private/LDP_publications_filtered_with_ca.csv`

- **Path**: `processed_data/private/LDP_publications_filtered_with_ca.csv`
- **Type**: Derived — private
- **Generated by**: `scripts/05_fetch_corresponding_authors.R`
- **Git-ignored**: Yes — contains LDP student names

`LDP_publications_filtered.csv` with two additional columns appended: corresponding author display name(s) and OpenAlex ID(s) fetched from the OpenAlex API. Used by script 07 to detect corresponding-author conflicts when selecting replacement comparators.

#### Additional columns (all others identical to `LDP_publications_filtered.csv`)

| Column | Type | Description | Nullable |
|---|---|---|---|
| `ca_display_names` | character | Display name(s) of corresponding author(s) as returned by OpenAlex; multiple authors separated by ` \| ` | Yes — `NA` if OpenAlex has no `is_corresponding` flag for this work |
| `ca_openalex_ids` | character | OpenAlex author entity URL(s) for corresponding author(s); multiple authors separated by ` \| ` | Yes — `NA` if OpenAlex has no `is_corresponding` flag for this work |

---

### `processed_data/comparator_publications_filtered_with_ca.csv`

- **Path**: `processed_data/comparator_publications_filtered_with_ca.csv`
- **Type**: Derived
- **Generated by**: `scripts/05_fetch_corresponding_authors.R`
- **Git-ignored**: No

`comparator_publications_filtered.csv` with the same two CA columns appended (see above). Used by scripts 07 and 08 to detect corresponding-author conflicts when selecting replacement comparators.

#### Additional columns

Same as `LDP_publications_filtered_with_ca.csv`: `ca_display_names`, `ca_openalex_ids`.

---

### `processed_data/rater_publications_pairs_dropped.csv`

- **Path**: `processed_data/rater_publications_pairs_dropped.csv`
- **Type**: Intermediate derived output
- **Generated by**: `scripts/06_drop_ineligible_pairs.R`
- **Git-ignored**: No — blinded; no PII

`rater_publications.csv` with PAIR006 and PAIR020 removed (4 rows dropped). Column schema is identical to `rater_publications.csv`.

---

### `processed_data/private/rater_key_pairs_dropped.csv`

- **Path**: `processed_data/private/rater_key_pairs_dropped.csv`
- **Type**: Intermediate derived output — private
- **Generated by**: `scripts/06_drop_ineligible_pairs.R`
- **Git-ignored**: Yes

`private/rater_key.csv` with PAIR006 and PAIR020 removed (4 rows dropped). Column schema is identical to `private/rater_key.csv`.

---

### `processed_data/rater_publications_final.csv`

- **Path**: `processed_data/rater_publications_final.csv`
- **Type**: Derived output
- **Generated by**: `scripts/07_replace_comparators.R`
- **Git-ignored**: No — blinded; no PII

`rater_publications_pairs_dropped.csv` with three ineligible comparator records (R75BCN, OZ2HY7, TILMW1) replaced by randomly selected eligible comparators. Adds one column (`original`) relative to `rater_publications.csv`.

#### Column schema

All columns from `rater_publications.csv`, plus:

| Column | Type | Description | Nullable |
|---|---|---|---|
| `original` | character | `"yes"` if this record was present in the original `rater_publications.csv`; `"no"` if it is a correction-round replacement. Allows raters who scored from the original file to merge assessments with the updated file. | No |

---

### `processed_data/private/rater_key_final.csv`

- **Path**: `processed_data/private/rater_key_final.csv`
- **Type**: Derived output — private
- **Generated by**: `scripts/07_replace_comparators.R`
- **Git-ignored**: Yes

`private/rater_key_pairs_dropped.csv` with three ineligible comparator records replaced. Column schema is identical to `private/rater_key.csv`.

---

### `processed_data/rater_publications_final_v2.csv` ⟵ active rater file

- **Path**: `processed_data/rater_publications_final_v2.csv`
- **Type**: Derived output
- **Generated by**: `scripts/08_replace_inaccessible.R`
- **Git-ignored**: No — blinded; no PII

`rater_publications_final.csv` with the inaccessible article (N3V3VY, PAIR017, McGill 2022) replaced by a randomly selected eligible comparator. This is the **current active file for raters**. Column schema is identical to `rater_publications_final.csv` (includes the `original` column).

---

### `processed_data/private/rater_key_final_v2.csv` ⟵ active key file

- **Path**: `processed_data/private/rater_key_final_v2.csv`
- **Type**: Derived output — private
- **Generated by**: `scripts/08_replace_inaccessible.R`
- **Git-ignored**: Yes

`private/rater_key_final.csv` with the inaccessible article replaced. This is the **current active key file**. Column schema is identical to `private/rater_key.csv`.
...
| **Post-hoc correction files (scripts 05–08)** | | | |
| `processed_data/private/LDP_publications_filtered_with_ca.csv` | Yes | Yes | `scripts/05_fetch_corresponding_authors.R` |
| `processed_data/comparator_publications_filtered_with_ca.csv` | No | No | `scripts/05_fetch_corresponding_authors.R` |
| `processed_data/rater_publications_pairs_dropped.csv` | No | No | `scripts/06_drop_ineligible_pairs.R` |
| `processed_data/private/rater_key_pairs_dropped.csv` | Yes | Yes | `scripts/06_drop_ineligible_pairs.R` |
| `processed_data/rater_publications_final.csv` | No | No | `scripts/07_replace_comparators.R` |
| `processed_data/private/rater_key_final.csv` | Yes | Yes | `scripts/07_replace_comparators.R` |
| `processed_data/rater_publications_final_v2.csv` ⟵ **active** | No | No | `scripts/08_replace_inaccessible.R` |
| `processed_data/private/rater_key_final_v2.csv` ⟵ **active** | Yes | Yes | `scripts/08_replace_inaccessible.R` |
